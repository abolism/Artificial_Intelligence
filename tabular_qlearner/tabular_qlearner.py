...
class QLearner(rl_agent.AbstractAgent):
    ...
    def step(self, time_step, is_evaluation=False, top1=False):
        ...
        if self._prev_info_state and not is_evaluation:
            ...
            prev_q_values = [self._q_values[self._prev_info_state][a] for a in legal_actions]
            # prev_max_q_value = max(prev_q_values)
            # #but running the above line of code gives me the following error:
            # #ValueError: max() arg is an empty sequence
            # #to fix this we must change the code to the following:
            if len(prev_q_values) == 0:
                prev_max_q_value = 0
            else:
                prev_max_q_value = max(prev_q_values)
                
            # Update the value of the previous state.
            self._q_values[self._prev_info_state][self._prev_action] += self._step_size * (
                reward + self._discount_factor * prev_max_q_value - self._q_values[self._prev_info_state][self._prev_action]
            )
