...
#some parts of the code below are provided using some help from github copliot
...
class SimulatedAnnealing:
    ...
    def random_state_generator(self, n: int) -> np.ndarray:
        #code to generate random state in simulated annealing algorithm
    ...
    def neighbor_state_generator(self, state: np.ndarray, S: np.ndarray, T: int, prob: float = 0.5) -> np.ndarray:
        ...
        - neighbor state
        """
        # rand = np.random.randint(0,len(state))
        # state[rand] = 1 - state[rand]
        # pCheck = random.random()
        # if pCheck <= prob:
        #     rand = np.random.randint(0,len(state))
        #     state[rand] = 1 - state[rand]
        # if self.is_feasible(state, S, T):
        #     return state
        # else:
        #     return self.neighbor_state_generator(state, S, T, prob)

        neighbor = state.copy()
        while True:
            rIndex = np.random.randint(len(state))
            neighbor[rIndex] = 1 - neighbor[rIndex]
            if np.random.rand() <= prob:
                rIndex = np.random.randint(len(state))
                neighbor[rIndex] = 1 - neighbor[rIndex]
            if self.is_feasible(neighbor, S, T):
                return neighbor

        # nState = state
        # while not self.is_feasible(nState, S, T):
    ...
    def prob_accept(self, state_cost: int, next_state_cost: int, temperature: float) -> float:
        ...
        """
        if next_state_cost - state_cost > 0:
    ...
    def run_algorithm(self, S: np.ndarray, T: int, neigbor_prob: float = 0.5, stopping_iter: int = 3000, temperature: float = 30):
        ...
        best_solution = None
        records = []
        initial_state = self.random_state_generator(len(S))
        current_state = initial_state
        best_soution = initial_state
        best_cost = self.cost_function(initial_state, S, T)
        ...
        for i in tqdm(range(stopping_iter)):
            ...
            current_state_cost,neighbor_state_cost = self.cost_function(current_state, S, T) , self.cost_function(neighbor_state, S, T)
            if neighbor_state_cost <= current_state_cost:
                ...
                pCheck = random.random()
                if pCheck <= self.prob_accept(current_state_cost, neighbor_state_cost, temperature):
                    current_state = neighbor_state
